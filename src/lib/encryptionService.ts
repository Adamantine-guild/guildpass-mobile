...
export class EncryptionError extends Error {
  constructor(message: string, public readonly code: EncryptionErrorCode) {
    super(message);
...
export class EncryptionService {
  ...
  async decrypt<T = any>(
    ...
    try {
      ...
      if (!authTag || authTag.length !== this.AUTH_TAG_LENGTH) {
        throw new EncryptionError(`Authentication tag must be ${this.AUTH_TAG_LENGTH} bytes`, 'INVALID_DATA');
      }
   ...
   */
  async verifyRoundTripIntegrity(
    data: string,
    key: ArrayBuffer | CryptoKey
  ): Promise<boolean> {
    try {
  ...
  async validateKey(key: ArrayBuffer | CryptoKey): Promise<boolean> {
    try {
      const cryptoKey = await this.normalizeKey(key);
      return cryptoKey.algorithm.name === this.ALGORITHM && 
  ...
  private updatePerformanceMetrics(operation: 'encrypt' | 'decrypt', timeMs: number): void {
    ...
    // Update averages
    const encryptCount = this.performanceMetrics.totalOperations - 
      (this.performanceMetrics.totalOperations / 2);
