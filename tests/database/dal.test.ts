...
describe("Guild CRUD", () => {
  it("upserts and retrieves a guild", async () => {
    await dal.upsertGuild(db as never, {
      id: "guild-1", name: "Test Guild", description: "Desc",
      icon_url: null, chain_id: 1,
  ...
  it("deletes a guild", async () => {
    await dal.upsertGuild(db as never, {
      id: "gx", name: "X", description: null, icon_url: null,
      chain_id: 1, raw_json: "{}",
      updated_at: new Date().toISOString(),
    });
...
describe("Wallet CRUD", () => {
  ...
  it("removes a wallet", async () => {
    ...
    expect(await dal.getWalletByAddress(db as never, "0x1234567890abcdef1234567890abcdef12345678")).toBeNull();
  });
  ...
  it("updates wallet label", async () => {
    ...
    const w = await dal.getWalletByAddress(db as never, "0x1234567890abcdef1234567890abcdef12345678");
...
describe("Access checks", () => {
  it("inserts and retrieves checks", async () => {
    ...
    await dal.insertAccessCheck(db as never, {
      id: "c1", wallet_address: "0x1234567890abcdef1234567890abcdef12345678",
      guild_id: "g1", resource_id: "r1", status: "granted",
      reason: null, matched_roles_json: "[]", required_roles_json: "[]",
      checked_at: now, created_at: now,
    });
    const checks = await dal.getAccessChecksByWallet(db as never, "0x1234567890abcdef1234567890abcdef12345678");
  ...
  it("paginates checks", async () => {
    ...
    for (let i = 0; i < 25; i++) {
      await dal.insertAccessCheck(db as never, {
        id: `c${i}`, wallet_address: "0x1234567890abcdef1234567890abcdef12345678",
        guild_id: "g1", resource_id: `r${i}`, status: "granted",
        reason: null, matched_roles_json: "[]", required_roles_json: "[]",
        checked_at: now, created_at: now,
      });
    }
    const p1 = await dal.getAccessChecksByWallet(db as never, "0x1234567890abcdef1234567890abcdef12345678", 10, 0);
    const p2 = await dal.getAccessChecksByWallet(db as never, "0x1234567890abcdef1234567890abcdef12345678", 10, 10);
  ...
  it("counts checks", async () => {
    ...
    for (let i = 0; i < 10; i++) {
      await dal.insertAccessCheck(db as never, {
        id: `cc${i}`, wallet_address: "0x1234567890abcdef1234567890abcdef12345678",
        guild_id: "g1", resource_id: "r", status: "granted",
        reason: null, matched_roles_json: "[]", required_roles_json: "[]",
        checked_at: now, created_at: now,
      });
    }
  ...
  it("deletes checks by wallet", async () => {
    const now = new Date().toISOString();
    await dal.insertAccessCheck(db as never, {
      id: "delme", wallet_address: "0x1234567890abcdef1234567890abcdef12345678",
      guild_id: "g1", resource_id: "r", status: "granted",
      reason: null, matched_roles_json: "[]", required_roles_json: "[]",
      checked_at: now, created_at: now,
    });
    await dal.deleteAccessChecksByWallet(db as never, "0x1234567890abcdef1234567890abcdef12345678");
    expect(await dal.getAccessChecksByWallet(db as never, "0x1234567890abcdef1234567890abcdef12345678")).toEqual([]);
  });
});
...
describe("Bulk operations", () => {
  it("bulk inserts guilds", async () => {
    const guilds = Array.from({ length: 50 }, (_, i) => ({
      id: `b${i}`, name: `Bulk ${i}`, description: null,
      icon_url: null, chain_id: 1, raw_json: "{}",
      updated_at: new Date().toISOString(),
    }));
...
describe("Synthetic data", () => {
  it("generates dataset under performance target", async () => {
    ...
    await dal.generateSyntheticData(db as never, {
      guildCount: 100, rolesPerGuild: 5,
      walletCount: 10, checksPerWallet: 5,
    });
